...
`https://matplanlegger.rusti.no/?profile=<profilkode>`
...
Legg Worker-URL inn før `app.js` i `index.html`:

```html
<script>
  window.MATPLAN_API = 'https://matplanlegger-api.<din-subdomain>.workers.dev';
</script>
...
`matplanlegger.rusti.no`

På GitHub: **Settings → Pages → Custom domain → `matplanlegger.rusti.no`**.
